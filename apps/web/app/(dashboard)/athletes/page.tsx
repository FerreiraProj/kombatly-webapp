...
export default function AthletesPage() {
  ...
  function reload() {
    clubsApi.getMyAthletes().then(setAthletes).catch(() => {});
  }
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      setSuccessMsg(`${created.firstName} ${created.lastName} added successfully`);
      reset();
      setShowForm(false);
      setTimeout(() => setSuccessMsg(''), 3000);
    } catch (e: any) {
      const msg = e?.response?.data?.message;
      setSubmitError(Array.isArray(msg) ? msg.join(', ') : msg ?? 'Failed to add athlete');
    }
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Club Roster</p>
          <h1 className="font-heading text-4xl text-foreground sm:text-5xl">ATHLETES</h1>
        </div>
        <button
        ...
        >
          ...
          <span className="hidden sm:inline">{showForm ? 'Cancel' : 'Add Athlete'}</span>
        </button>
      </div>
      ...
      {showForm && (
        ...
        >
          <h3 className="font-heading text-lg text-foreground">NEW ATHLETE</h3>
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-16 text-center">
          ...
          {!search && (
            <p className="mt-1 text-sm text-muted-foreground">
              Add athletes to your club to start registering them in tournaments
            </p>
          )}
        </div>
      ) : (
          ...
          <ul className="divide-y divide-border">
            {filtered.map((a) => (
              ...
              >
                ...
                <div className="col-span-2 hidden sm:flex items-center text-muted-foreground text-xs">
                  {a.gender?.code === 'M' ? 'Male' : a.gender?.code === 'F' ? 'Female' : '—'}
                </div>
              </li>
            ))}
          </ul>
      ...
      <p className="text-xs text-muted-foreground text-center">
        {athletes.length} athlete{athletes.length !== 1 ? 's' : ''} in your club
      </p>
    </div>
  );
}
