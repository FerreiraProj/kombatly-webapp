...
import { Search, MapPin, Calendar, Clock, Users, Trophy, ChevronLeft, ChevronRight } from 'lucide-react';
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === 'PUBLIC') return <span className="rounded px-2 py-0.5 text-xs font-semibold bg-green-500/10 text-green-400 border border-green-500/20">ABERTO</span>;
    ...
    <div className={`rounded-lg border bg-surface flex flex-col transition-colors hover:border-primary/50 ${isLive ? 'border-primary/40' : 'border-border'}`}>
      <div className="p-5 flex-1 space-y-3">
        <div className="flex items-start justify-between gap-2">
          <p className="text-xs text-muted-foreground">{flag} {countryName}{event.city ? ` · ${event.city}` : ''}</p>
        ...
        {isOpen && !deadlinePassed && (
          <p className="text-xs text-primary font-medium">Inscrições até {formatDate(event.registrationDeadline)}</p>
        )}
      ...
      <div className="px-5 pb-5">
        <Link href={`/t/${event.slug}`} className="block w-full rounded border border-border py-2 text-center text-xs font-semibold text-foreground hover:border-primary hover:text-primary transition-colors">
          Ver Torneio →
        </Link>
      </div>
    </div>
...
export function EventsContent() {
  ...
  useEffect(() => { setPage(1); }, [country, status, debouncedSearch]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        {total > 0 && !loading && (
          <p className="mt-1 text-sm text-muted-foreground">{total} torneios encontrados</p>
        )}
      </div>
      ...
      {error ? (
        <div className="flex flex-col items-center gap-4 py-16 text-center">
          ...
          <button onClick={fetchEvents} className="rounded bg-primary px-4 py-2 text-sm text-white hover:bg-red-700">Tentar novamente</button>
        </div>
      ) : loading ? (
